...
- 初始资源池固定为 2+6+4（2026-09-08 扩编，旧口径 2+4+2）：R1–R2 侦察、E1–E6 灭火、S1–S4 支援（S3/S4 multi_role 可参与灭火）；根据约束动态出动，不是固定三架。
...
AnalysisStore 使用 SQLite 写穿持久化（`data/analysis_store.db`，git 忽略），重启后任务、方案版本、事件和资源锁可恢复。视觉双真源已上线：VLM=glm-4.6v-flash 直连、YOLO=自训 yolo11n-dfire-v1 独立服务（`FIRE_YOLO_ENDPOINT` 门控，见 `yolo_server/README.md`）；规则快照在检测零框/服务失败时如实回落并标注。生产级 GIS 与飞控属于后续适配能力。
