...
# 默认将 DEM 文件与本脚本放在同一目录；也可在调用时传入 dem_path
DEFAULT_DEM_PATH = Path(__file__).resolve().parent / "N32E118.hgt"
...
def get_terrain(latitude, longitude, dem_path, window_size=5):
    ...
    with rasterio.open(dem_path) as dataset:
        ...
        half = window_size // 2
        window = Window(
            col - half, row - half, window_size, window_size
        )
    ...
    return {
        ...
        "downslope_deg": (
            None if down_deg is None else round(down_deg, 1)
        ),
        "downslope_direction": degree_to_direction(down_deg),
        "upslope_deg": (
            None if up_deg is None else round(up_deg, 1)
        ),
...
def get_weather(latitude, longitude):
    ...
    response = requests.get(
        OPEN_METEO_URL, params=params, timeout=20
    )
    response.raise_for_status()
    ...
    return {
        ...
        "wind_from_direction": degree_to_direction(
            wind_from_deg
        ),
        "wind_to_deg": (
            None if wind_to_deg is None else round(wind_to_deg, 1)
        ),
...
def get_landcover(
    ...
    if item is None:
        raise RuntimeError(
            "当前位置没有找到 ESA WorldCover 数据"
        )

    asset = (
        item.assets.get("map")
        or next(iter(item.assets.values()))
    )
    ...
    burnable_ratio = float(
        np.isin(valid, list(BURNABLE_NATURAL)).sum()
        / valid.size
    )

    return {
        "center_class": WORLD_COVER_CLASSES.get(
            center, "Unknown"
        ),
        "dominant_class": WORLD_COVER_CLASSES.get(
            dominant, "Unknown"
        ),
        "burnable_ratio": round(burnable_ratio, 3),
        "fuel_possible": (
            burnable_ratio >= min_burnable_ratio
        ),
    }
...
def line_distance(latitude, longitude, geometry):
    ...
    fire_x, fire_y = to_utm.transform(
        longitude, latitude
    )
...
def get_environment(
    ...
    dem_path:
        可选。未提供时默认读取与本脚本同目录的 N32E118.hgt。
    """
...
def print_environment_brief(result):
    ...
    if terrain["status"] == "ok":
        print(
            f"地形：海拔 {terrain['elevation_m']} m，"
            f"坡度 {terrain['slope_deg']}°，"
            f"上坡 {terrain['upslope_direction']}"
        )
    else:
        print("地形：失败 -", terrain["error"])

    weather = result["weather"]
    if weather["status"] == "ok":
        print(
            f"天气：{weather['temperature_c']} ℃，"
            f"湿度 {weather['relative_humidity_pct']}%，"
            f"风速 {weather['wind_speed_m_s']} m/s，"
            f"{weather['wind_from_direction']} → "
            f"{weather['wind_to_direction']}"
        )
    else:
        print("天气：失败 -", weather["error"])

    landcover = result["landcover"]
    if landcover["status"] == "ok":
        print(
            f"植被：{landcover['dominant_class']}，"
            f"可燃覆盖比例 "
            f"{landcover['burnable_ratio']:.0%}"
        )
    else:
        print("植被：失败 -", landcover["error"])
    ...
    if road["status"] == "ok":
        access = (
            road["nearest_vehicle_access_candidate"]
            or road["nearest_transport"]
        )
        if access:
            print(
                f"道路：{access['name']}，"
                f"约 {access['distance_m']} m"
            )
        else:
            print("道路：搜索范围内未找到")
    else:
...
# 直接运行本文件时用于快速测试；被 import 时不会自动执行
if __name__ == "__main__":
    demo = get_environment(
        latitude=32.0725,
        longitude=118.8406,
    )
    print_environment_brief(demo)
