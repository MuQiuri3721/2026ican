...
| 调度方案 | `plan_id`、`task_id`、`plan_version`、`selected_uavs`、`tasks`、`material_module`、`fire_load_flp`、`growth_rate_per_hour`、`growth_baseline_flp`、`replan_trigger_baseline_flp`、`battery_plan`、`people_branch`、`estimated_control_time`、`feasibility`、`resource_gap`、`alternative_plan`、`replan_trigger`、`scoring`、`control_verdict`、`control_reason_code`、`environment_snapshot_id`、`water_source_plan.source_id` | 时间为 min 区间；W20=L、C6=kg | 确定性调度链（`deterministic_v1_dispatch`）；OPT-P0-04 起方案携带三态 `control_verdict`（can_control/maintain_only/cannot_control）与同源原因码 `control_reason_code`（controlled_within_time/time_limit_exceeded/suppression_slow_within_window/suppression_insufficient/agent_insufficient/soc_below_return/no_firefighting_units——「时间不够」与「压制不足」分离）；OPT-P2-02 起 `water_source_plan.source_id` 为已批准水源（osm_id 优先，场景回退 scene:name#index），执行侧失效不换候选；OPT-P2-04 起 `environment_snapshot_id` 绑定输入证据（内容哈希寻址，见 `environment_snapshots`） |
