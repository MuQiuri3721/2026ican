...
| 调度方案 | `plan_id`、`task_id`、`plan_version`、`selected_uavs`、`tasks`、`material_module`、`fire_load_flp`、`growth_rate_per_hour`、`growth_baseline_flp`、`replan_trigger_baseline_flp`、`battery_plan`、`people_branch`、`estimated_control_time`、`feasibility`、`resource_gap`、`alternative_plan`、`replan_trigger`、`scoring`、`control_verdict`、`control_reason_code`、`environment_snapshot_id`、`water_source_plan.source_id`、`water_source_plan.candidates[]` | 时间为 min 区间；W20=L、C6=kg | 确定性调度链（`deterministic_v1_dispatch`）；OPT-P0-04 起方案携带三态 `control_verdict`（can_control/maintain_only/cannot_control）与同源原因码 `control_reason_code`（controlled_within_time/time_limit_exceeded/suppression_slow_within_window/suppression_insufficient/agent_insufficient/soc_below_return/no_firefighting_units——「时间不够」与「压制不足」分离）；OPT-P2-02 起 `water_source_plan.source_id` 为已批准水源；FE-73 起规划/执行两端统一水源 ID（osm_id → 库存 id → scene:name#idx 共享实现），`water_source_plan.candidates[]` 持久化全部通过六条件的候选（就近排序，base 模式为单元素），执行端按列表逐个降级、全败才置 water_source_invalid 触发重规划（未经批准的库存候选仍不放行）；OPT-P2-04 起 `environment_snapshot_id` 绑定输入证据（内容哈希寻址，见 `environment_snapshots`） |
| 轮次 FLP 账本 | `rounds[].after.flp_ledger`（`before_flp`/`growth_flp`/`suppression_flp`/`net_change_flp`/`after_flp`/`minutes`/`per_minute[]`）、`rounds[].observation_adjustment_flp` | FLP，保留 4 位 | OPT-P0-03：全部出自统一分钟核心（advance_one_minute），监测端只求和；无观测校正时 `after = before + growth − suppression`；负荷清零轮有效压制截断为 before+growth；`observation_adjustment_flp` 为图像观测造成的状态校正（本轮 before − 上轮 after），不混入自然增长 |
| 库存审计 | `c6_scrapped_kg` | kg | OPT-P0-04：C6 整型模块未用余量随报废的去向记录（水剂回注基地、C6 报废，公斤账闭合有去向） |
