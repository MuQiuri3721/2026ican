...
| 401 | 指挥员口令门（FE-71）：配置环境变量 `FIREOPS_COMMANDER_TOKEN` 后，全部 POST 变更路由要求请求头 `X-Commander-Token` 同值（研判/上传/审批/重规划/轮次/监测/Skill 兼容通道/问答），缺失或错误 401；未配置该变量 = 开放模式，GET 只读端点始终开放。前端保存口令后全请求自动附带 |
| 413 | 上传超过 200 MB |
...
- 列表：`{"items": [AnalysisEnvelope...]}`（新任务在前）。查询参数 `limit`（1–1000，截取最新 N 条）与 `slim=1`（只含 analysis_id/status/created_at/updated_at/monitor_round/resource_locks/input 摘要字段 + `summary` 归档行摘要 `{level, level_label, fire_area_m2, fire_load_flp, control_verdict, people_status}`（2026-09 UI 新增），不含 result/stages 等重负载；完整信封经 `GET /api/analyze/{id}` 按需获取）。缺省不传时行为不变（全量完整信封）。
