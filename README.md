...
> **一句话**：真实 GIS/气象/影像数据 + 确定性规则引擎算所有安全关键数字 + 六角色多智能体组织与建议 + 人在回路审批门，构成一个可审计、可复现、防幻觉的森林火情研判与无人机调度闭环平台——209 项后端回归与四层端到端测试全绿。
...
| 视觉理解 | 已接入 | VLM=glm-4.6v-flash 真实接入（图片/多图/视频自动抽帧，含契约守卫与限流降级）；YOLO=自训 yolo11n-dfire-v1 已接入实时服务（D-Fire，test mAP50 0.665），PWM-Net 到位同端点切换 |
| 指挥员问答 | 已接入 | ChatPanel 接地任务实时数据（库存/方案），数字审计防编造 |
| 三维与广播 | 已接入 | 真实 DEM 三维地形；疏散路线 Edge TTS 语音广播 |
| 测试资产 | 已接入 | pytest 209 项；E2E R1-R23 全量套件（_suite_runner.py，2026-09-23 实跑 22/23 绿 + ENV-1 归属 B 待办）+六场景+边界扫描 44 项+API 全功能 39 项；OPT 方案回归矩阵（docs/下一阶段优化与修正方案.md） |
...
- [需求与设计思路](docs/requirements.md)
- [系统架构](docs/architecture.md)
- [系统架构与算法总览](docs/architecture.md)
