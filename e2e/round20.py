...
def main() -> int:
    ...
    try:
        ...
        # 投影样式真实生效（强断言）：hero 结论字号 14px→24px（FE-60 曾只挂类无样式，投影是空操作）
        hero_px = page.evaluate("parseFloat(getComputedStyle(document.querySelector('.hero-footer strong')).fontSize)")
        ok &= report(20, "投影字号放大", hero_px >= 22, f"hero-verdict={hero_px}px")
