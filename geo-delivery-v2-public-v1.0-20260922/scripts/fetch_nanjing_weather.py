...
def nanjing_representative_point(boundary_path: Path) -> tuple[float, float, str]:
    ...
    metric = confirmed.to_crs("EPSG:32650").geometry.iloc[0]
    point = gpd.GeoSeries([metric.representative_point()], crs="EPSG:32650").to_crs("EPSG:4326").iloc[0]
    if not confirmed.geometry.iloc[0].covers(point):
        raise ValueError("Representative weather point is not inside the confirmed Nanjing AOI")
    return round(point.y, 6), round(point.x, 6), "representative_point_of_confirmed_nanjing_aoi_in_epsg32650"
...
def main() -> None:
    ...
    latitude, longitude, point_method = nanjing_representative_point(boundary_path)
    ...
    current = {
        "dataset_id": "open_meteo_nanjing_current_20260921",
        "region_id": "nanjing",
    ...
    forecast = {
        "dataset_id": "open_meteo_nanjing_forecast_24h_20260921",
        "region_id": "nanjing",
        "request_point": {"latitude": latitude, "longitude": longitude, "crs": "EPSG:4326"},
        "returned_grid": {
            "latitude": forecast_raw.get("latitude"),
    ...
    metadata = {
        "dataset_id": "open_meteo_nanjing_weather_delivery_20260921",
        "region_id": "nanjing",
        "scope": "one representative query point for the confirmed Nanjing AOI",
        "request_point": {
            "latitude": latitude,
            "longitude": longitude,
            "crs": "EPSG:4326",
