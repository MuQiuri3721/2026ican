...
function setSimSpeed(speed) {
  ...
  if (!mission?.active) { simSpeed.value = speed; return }
  const elapsedMin = (Date.now() - mission.startedAt) / (mission.msPerMin || MISSION_MS_PER_MIN)
  mission.msPerMin = MISSION_MS_PER_MIN / speed
  mission.startedAt = Date.now() - elapsedMin * mission.msPerMin
...
function startAutoSim() {
  stopAutoSim()
  missionTimer = window.setInterval(() => { runMonitor(true) }, MISSION_ROUND_MS / (simSpeed.value || 1))
}
...
<template>
  <div class="app-shell">
    ...
    <div v-if="alarms.length" class="alarm-stack" role="alert">
      ...
      <header><div><div class="breadcrumb">COMMAND CENTER <span>/</span> {{ (viewTab === 'agents' && activeTab === 'logs' ? 'agents' : activeTab).toUpperCase() }}</div><h1>森林火灾救援工作台</h1><p>多源感知 · 智能研判 · 集群调度 · 闭环处置</p></div><div class="header-actions"><button :class="['icon-btn', { proj: projection }]" :aria-pressed="projection" title="大屏投影模式（放大字号与数据对比度）" @click="toggleProjection"><MonitorUp :size="18" /></button><button class="icon-btn" title="查看任务事件通知" @click="selectNav('logs')"><Bell :size="18" /><i></i></button><span class="status-tag" :class="llmInfo?.available ? '' : 'orange'" :title="llmInfo?.available ? 'GLM 在线研判' : 'LLM 未配置 · 确定性降级'"><Activity :size="14" /> LLM {{ llmInfo?.available ? '在线' : '离线' }}</span><div class="utc">本地时间<br><strong>{{ todayLabel }}</strong></div></div></header>
      ...
      <div v-if="activeTab === 'command'" class="dashboard">
        <section class="hero-panel"><div class="panel-heading"><h2>{{ scene.incident }}</h2><span class="severity"><span></span>{{ result.fire_assessment.label }}</span></div><p class="hero-note">火点 <b>{{ scene.coordinates }}</b> · 现场风 <b>{{ result.environment.wind_direction || '—' }} {{ result.environment.wind_speed ?? '—' }} m/s</b> · 最近水源 <b>{{ result.environment.nearest_water?.name || '—' }} {{ result.environment.nearest_water?.distance_m ?? result.environment.nearest_water_distance_m ?? '' }}</b></p><div v-if="result?.fire_assessment" class="fire-sense"><div :class="['fs-level', 'lv-' + fireLevelNum]"><b>{{ result.fire_assessment.label }}</b><span>{{ fireLevelWord }}</span></div><div class="fs-cell"><small>火势负荷 FLP</small><b>{{ formatNumber(fireFlpNow) }}</b><em v-if="fireTrend != null" :class="fireTrend < 0 ? 'down' : 'up'">{{ fireTrend < 0 ? '▼' : '▲' }} {{ Math.abs(Math.round(fireTrend * 100) / 100) }} 较上轮</em><em v-else>首轮基准</em></div><div class="fs-cell"><small>过火面积</small><b>{{ formatNumber(result.fire_assessment.fire_area_m2) }}</b><em>m²</em></div><div class="fs-cell"><small>蔓延速率</small><b>{{ Math.round((result.fire_assessment.growth_rate || 0) * 100) }}%</b><em>/h</em></div><div v-if="fireTrendText" :class="['fs-verdict', fireTrendText.down ? 'down' : 'up']">{{ fireTrendText.text }}<small>{{ fireTrendText.detail }}</small></div></div><div class="hero-footer" :class="controlVerdictView.tone"><div><small>当前处置结论</small><strong>{{ controlVerdictView.hero }}</strong></div><div class="hero-stat"><small>预计处置时间</small><strong>{{ result.dispatch_plan.estimated_minutes ?? '—' }} <em>MIN</em></strong></div><div class="hero-stat"><small>下次评估</small><strong>05 <em>MIN</em></strong></div></div></section>
