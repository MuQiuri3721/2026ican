...
function generateScenario() {
  ...
  scenario.value = { fireOrigin, fireGps, areaM2, growthRate, people, failureRound }
...
async function startScenarioSimulation() {
  ...
  try {
    const response = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
      ...
      scenario: { fire_origin: scenario.value.fireOrigin, fire_area_m2: scenario.value.areaM2, growth_rate: scenario.value.growthRate, uav_failure_round: scenario.value.failureRound },
    }) })
    if (!response.ok) {
      const detail = await response.json().catch(() => ({}))
    ...
    <main :class="{ 'main-map': activeTab === 'map' }">
      ...
      <div class="map-taskbar" role="toolbar" aria-label="任务控制">
        ...
        <template v-else-if="scenario && !analysisResult">
          <span class="scr-hint">火情已生成 · 面积 {{ scenario.areaM2 }}m² · 增长率 {{ scenario.growthRate }}/h · 人员{{ scenario.people === 'confirmed' ? '在场' : scenario.people === 'absent' ? '不在场' : '情况不明' }}{{ scenario.failureRound ? ` · ⚔ 单机失能演练（第 ${scenario.failureRound} 轮）` : '' }}</span>
