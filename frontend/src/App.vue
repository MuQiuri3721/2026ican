...
async function selectHistoryTask(task) {
  ...
  // FE-88：恢复任务是来看结果的——工具抽屉（上传面板+演训 714px）默认收起，
  // 需要「清空并重新接入」时再点开
...
watch(monitorResult, (m) => {
  ...
  alarms.value.unshift({
    ...
    text: [triggers.map(replanTriggerLabel).join('、'), emergency.length ? `应急机 ${emergency.join('、')}` : ''].filter(Boolean).join(' · '),
  })
...
// FE-90 任务锁定显示口径：仅活动态任务（待确认/执行/重规划）显示「是 · 方案」；
// 已完成/已终止后端已释放锁（resource_locks=[]），继续显示会误导库存判断
const activeLockIds = computed(() => {
  const st = analysisEnvelope.value?.status
  const active = ['awaiting_confirmation', 'approved', 'executing', 'replanning'].includes(st)
  return active ? (analysisResult.value?.dispatch_plan?.selected_uavs || []) : []
})
...
<template>
  ...
  <aside class="sidebar">
    ...
    <main :class="{ 'main-map': activeTab === 'map', 'main-command': activeTab === 'command' }">
